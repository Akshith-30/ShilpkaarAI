...
router.get('/artisan/allorders', auth, async (req, res) => {
  try {
    ...
    const orders = await Order.find({
      'items.product': { $in: await Product.find({ artisan: req.userId }).distinct('_id') }
    })
...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    const isArtisan = user.role === 'artisan' && 
      order.items.some(item => item.product.artisan.toString() === req.userId);
...
router.put('/:id/status', auth, [
  ...
  try {
    ...
    const user = await User.findById(req.userId);
    const isCustomer = order.customer.toString() === req.userId;
    const isArtisan = user.role === 'artisan' && 
      order.items.some(item => item.product.artisan.toString() === req.userId);
